...
def calculate_person_consumption(
    persons: int, config: Dict[str, Any] = DEFAULTS
) -> float:
    return persons * config["consumption_per_person_per_day"]


def calculate_ev_consumption(
    ev_count: int, temperature: float, config: Dict[str, Any] = DEFAULTS
) -> float:
    temperature_factor = (
        1
...
def calculate_heat_pump_consumption(
    has_heat_pump: bool, temperature: float, config: Dict[str, Any] = DEFAULTS
) -> float:
...
def calculate_current_consumption(
    ...
    ev_consumption = calculate_ev_consumption(
        house.get("ev_count", 0), temperature, config
    )
...
def calculate_house_savings(
    ...
    grid_houses = [house for house in houses if house.get("part_of_grid", True)]
    ...
    total_remaining_demand = sum(house["remaining_demand"] for house in prepared_houses)
    ...
    for house in prepared_houses:
        ...
        external_energy_needed = house["remaining_demand"] - local_energy_received

        normal_cost = house["current_consumption"] * config["average_german_price"]
...
def calculate_grid_state(
    ...
    net_flow = calculate_net_flow(
        totals["total_generation"], totals["total_consumption"]
    )
