...
def calculate_current_price(
    ...
    grid_state = calculate_grid_state(
        houses=houses,
...
def generate_houses_details(only_savings_gains: bool = False, config: dict = DEFAULTS):
    ...
    for day in range(1, 366):
        ...
        for house in houses:
            current_generation = calculate_current_generation(
                house, solar_radiation, config=config
            )
            current_consumption = calculate_current_consumption(
                house, temperature, config=config
            )
